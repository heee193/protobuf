...
class PROTOBUF_EXPORT PROTOBUF_ALIGNAS(8) Arena final {
 public:
  ...
  //
  // If T satisfies the appropriate protocol (described below), it is deemed
  // "arena compatible" and handled more efficiently because this interface (i)
  // passes the arena pointer to the created object so that its sub-objects and
  // internal allocations can use the arena too, and (ii) elides the object's
  // destructor call when possible; e.g. protobuf messages, RepeatedField, etc.
  // Otherwise, the arena will invoke the object's destructor when the arena is
  // destroyed.
  //
  ...
  //   and a constructor callable with `Arena* arena, Args&&... args`, called
  //   when a T is allocated on an arena. If the second constructor is called
  //   with a null arena pointer, it must be equivalent to invoking the first
  ...
  //   trait is present in the type, then its destructor will not be called if
  //   and only if it was passed a non-null arena pointer. If this type trait is
  //   not present on the type, then its destructor is always called when the
  //   containing arena is destroyed.
  //
  // This protocol is implemented by all protobuf message classes as well as
  // protobuf container types like RepeatedPtrField and Map. The protocol is
  // internal to protobuf and is not guaranteed to be stable. Non-proto types
